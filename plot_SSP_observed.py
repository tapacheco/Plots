...
#
    ...
    plt.plot(schi05_w_m2, (schi05_flux/schi05_flux[index_S]), label='Schiavon+04',  c='grey', alpha=.9, linestyle='-', linewidth=3)
    axt.plot(wavelength_ssp, (flux_ssp/flux_ssp[index_ssp]), label='Modeled SSP', c='#ff00ff', linestyle='--', linewidth=3)
    ...
    axt.set_xlim(min(schi05_w_m2)-20,max(schi05_w_m2+20))
    axt.set_ylim(-.1,1.16)
    plt.legend(handlelength=1.5, fontsize=16, loc='best')
    plt.ylabel(r'Flux [erg cm$^{-2}$ s$^{-1}$ \AA $^{-1}$]', fontsize=16)
    ...
    plt.tick_params(direction='in', which='minor', length=5, width=1.2, \
                    bottom=True, top=True, left=True, right=True)
    plt.savefig(path+'_ssp.png', dpi=300, bbox_inches = 'tight')
